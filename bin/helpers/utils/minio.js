...
const objectUploadOld = async (bucketName, objectName, filePath) => {
  try {
    const isUploaded = await minioClient.putObject(bucketName, objectName, filePath);
    if (isUploaded) {
      return wrapper.data(isUploaded);
    }
  } catch (err) {
    logger.log('minioSdk.objectUpload.old', err.message, 'error upload object');
    return wrapper.error(err);
  }
};
...
const objectRemove = async (bucketName, objectName) => {
  try {
    ...
    logger.log('minioSdk-objectRemove', {bucketName, objectName}, 'success remove object');
    return wrapper.data(true);
  } catch (err) {
